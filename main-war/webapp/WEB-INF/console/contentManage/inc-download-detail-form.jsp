...
<div class="control-group">
	<label class="control-label" for="attrFile">附件</label>
	<div class="controls">
		<input type="text" name="attrFile" value="${article.attr['attrFile']}" placeholder="附件"/> 
		<input type="button" class="btn attrFileBtn" value="浏览附件" /> 
		<input type="button" class="btn btn-danger" value="删除" onclick="$('input:text[name=attrFile').val('');"/>
	</div>	
</div>
...
$(function() {
	...
	$('.attrFileBtn').on("click", function(){
		var finder = new CKFinder();
		finder.selectActionFunction = function(fileUrl) {
			$('input:text[name=attrFile]').val(fileUrl);
		};
		finder.popup();
	});
});
