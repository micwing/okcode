...
<%
	...
	int pagingSize = Integer.parseInt(request.getParameter("_paging_size"));
	int pagingRange = Integer.parseInt(request.getParameter("_paging_range"));
    ...
    if (number <= pagingRange) {
    	min = 1;
    	max = (max + ((pagingRange+1) - number)) >= totalPages ? totalPages : (max + ((pagingRange+1) - number));
    }
    if (number > totalPages - pagingRange) {
    	max = totalPages;
    	min = (min - (number - (totalPages - pagingRange))) <= 0 ? 1 : (min - (number - (totalPages - pagingRange)));
    }
%>

		<ul>
			<c:if test="${!page.firstPage}">
				<li title="首页"><a href="<%=pagingUrl%>&page.page=1&page.size=<%=pagingSize%>">&lt;&lt;</a></li>
			...
			<c:forEach begin="<%=min %>" end="<%=max %>" step="1" var="i">
				<c:if test="${i != (page.number+1)}">
					<li title="第${i}页"><a href="<%=pagingUrl%>&page.page=${i}&page.size=<%=pagingSize%>">${i}</a></li>
				</c:if>
				<c:if test="${i == (page.number+1)}">
			...
			<c:if test="${!page.lastPage}">
				...
				<li title="尾页"><a href="<%=pagingUrl%>&page.page=${page.totalPages}&page.size=<%=pagingSize%>">&gt;&gt;</a></li>
			</c:if>
			<%-- <c:if test="${page.lastPage}">
